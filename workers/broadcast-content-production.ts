...
const outcomes=['PRODUCED_TO_SHORT_REQUIRED','PRODUCED_TO_OWNER_GATE','NO_ELIGIBLE_WORK','BLOCKED_COORDINATION_NOT_SHARED','BLOCKED_GRANT','CLAIM_LOST','FAILED_DEFINITE','UNKNOWN'] as const;
...
function parse(input:unknown):Reply {
 ...
 if((r.outcome==='PRODUCED_TO_SHORT_REQUIRED'||r.outcome==='PRODUCED_TO_OWNER_GATE')&&(r.coordination!=='SHARED_TRANSACTIONAL'||!r.grant_valid||!r.eligible_work||r.production_enabled))throw Error('invalid success');
 if(r.outcome==='PRODUCED_TO_SHORT_REQUIRED'&&(r.final_state!=='SHORT_REQUIRED'||!r.spec_id||r.episode===null||r.initial_state!=='STILL_REQUIRED'))throw Error('invalid');
...
export async function runProduction(c:Config,transport:Transport):Promise<Report> {
 ...
 try {const u=new URL(c.origin);if(c.origin!=='https://aive.global'||u.origin!==c.origin||u.protocol!=='https:'||u.hostname!=='aive.global'||u.username||u.password||!uuid(c.grantId)||! /^[0-9]{1,24}:[0-9]{1,4}$/.test(c.runId)||! /^[!-~]{32,4096}$/.test(c.workerToken))throw Error();}
 ...
 try{const p=await call('preflight',15000);if(p.status===401||p.status===403)return report('BLOCKED_GRANT','AIVE_RESULT');if(p.status!==200)return report('BLOCKED_COORDINATION_NOT_SHARED','CONTRACT_INVALID');const r=parse(p.body);
  if(!r.production_enabled){if(r.outcome==='PRODUCED_TO_SHORT_REQUIRED'||r.outcome==='PRODUCED_TO_OWNER_GATE')throw Error('invalid preflight');return report(r.outcome??'BLOCKED_COORDINATION_NOT_SHARED','AIVE_RESULT',r);}
 }catch{return report('BLOCKED_COORDINATION_NOT_SHARED','CONTRACT_INVALID');}
 try{const p=await call('advance',330000);if(p.status!==200)return report('UNKNOWN','TRANSPORT_UNCERTAIN');const r=parse(p.body);if(r.production_enabled||r.outcome===null)throw Error();return report(r.outcome,'AIVE_RESULT',r);}
